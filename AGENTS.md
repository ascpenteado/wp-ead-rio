...
- `pnpm run dev` — boot local DDEV environment; `dev:logs` follows container logs.
- `pnpm run dev:down | dev:stop | dev:restart` — stop or restart the DDEV environment; `dev:clean` removes the project and database (use with caution).
